...
export async function pkpSearchFunc({ args }) {
  ...
  switch (params.get) {
    case 'authMethodScopes':
      const scopes = await getAuthMethodScopes(
        contractsClient,
        params.publicKey,
        params.authMethodId,
        params.type,
    ...
    case 'authMethods':
      const authMethods = await searchAuthMethods(
        contractsClient,
        params.publicKey
      );
    ...
    case 'tokenIds':
      const tokenIds = await getTokenIdsByAuthMethod(
        contractsClient,
        params.authMethodId,
        params.clientId,
        params.appId,
...
async function searchAuthMethods(client, pk) {
  ...
  let authMethods =
    await client.pkpPermissionsContract.read.getPermittedAuthMethods(tokenId);
  let authMethodObjs = [];
...
async function getAuthMethodScopes(
  client,
  pk,
  authMethodId,
  authMethodType,
  ...
  let tokenId = ethers.utils.keccak256(`0x${pk}`);
  let scopes =
    await client.pkpPermissionsContract.read.getPermittedAuthMethodScopes(
      tokenId,
      authMethodType,
      authMethodId,
      maxScope
...
async function getTokenIdsByAuthMethod(
  client,
  authMethodId,
  clientId,
  appId,
  ...
  if (!authMethodId) {
    authMethodId = ethers.utils.keccak256(
      ethers.utils.toUtf8Bytes(`${clientId}:${appId}`)
    );
  }

  let tokenIds =
    await client.pkpPermissionsContract.read.getTokenIdsForAuthMethod(
      type,
      authMethodId
    );
  tokenIds = {
